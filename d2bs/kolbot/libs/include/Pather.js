        ...
        if (me.inTown && Pather.wpAreas.indexOf(target.course[0]) > -1 && getWaypoint(Pather.wpAreas.indexOf(target.course[0]))) {
            ...
            case 40:
                if (Packet.checkQuest(6, 1) || Packet.checkQuest(6, 0)) {
					Town.move("warriv");		//Hint: bugfix start
					npc = getUnit(1, 155);		//
					if (npc) {					//
						npc.openMenu();			//
						Misc.useMenu(0x0D36);	//
					}							//bugfix end
                    //Quest.talkTo("warriv", "warriv");		//backup
                    //Quest.changeAct(2);
                }
                break;
            case 75:
				if (Packet.checkQuest(14, 0)) {		//Hint: bugfix jerhyn start
					Town.goToTown(2);				//
					Town.move("palace");			//
            ...
            case 109:
                if (Packet.checkQuest(26, 0)) {
                    Town.goToTown(4);
					Town.move(NPC.Tyrael);					//Hint: bugfix tyrael
                    ...
                    //Quest.talkTo("tyrael", "tyrael");		//backup
                    //Quest.changeAct(5);					//
                }
                break;        
            }
