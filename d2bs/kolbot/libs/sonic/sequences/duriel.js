...
function duriel(farm, clearPath) {
    let killDuriel = function () {
        ...
        for (i = 0; i < 900; i += 1) {
            Misc.townCheck();
			//ClassAttack.doCast(target, Config.AttackSkill[1], Config.AttackSkill[2]); backup
            ClassAttack.doAttack(target);	//Hint: fix static on classic
    ...
    if (me.area === durielTomb) {
        ...
        if (me.area === 73) {
            ...
            if (!farm) {
                ...
                if (me.inTown) {
                    if (!Packet.questMessage("palace", "jerhyn", 442) || !Packet.questMessage("meshif", "meshif", 450)) {
                        throw new Error("failed to send quest packets for act 3");
                    }
